...
let cardsTotal
let gameHistory = [];  // Stores game states for undo functionality
...
function moveCards(){
  ...
  gameHistory.push(JSON.parse(JSON.stringify(table)));
...
document.getElementById('btn-undo').addEventListener('click', () => {
  if (gameHistory.length > 0) {
    ...
    table = JSON.parse(JSON.stringify(lastState));
    
    // Redraw the game with the previous state
    redrawCards();
  }
});
